...
public class Job2 extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
		...
		job.setMapperClass(SimliaryMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(Text.class);
		//指定reduce程序所在的类以及keyout valueout类型IntWritable
		job.setReducerClass(SimilaryReducer.class);
	...
	static class SimliaryMapper extends Mapper<LongWritable, Text, Text, Text>{
		@Override
	...
	static class SimilaryReducer extends Reducer<Text, Text, Text, List<Text>>{
		@Override
